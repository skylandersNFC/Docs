...
* Also **not all Gen 2 CUID tags are the same** either, because it depends who manufactures them.
* **Most** Gen2 CUID tags will have `SAK: 08` and `ATQA: 0F01`. Those will be recognized as "**NXP Mifare Classic (MF1S50)**".
* **Some** Gen2 CUID tags will have `SAK: 08` and `ATQA: 0004`. Those will be recognized as "**Unknown Mifare Classic IC**".
* However **others** Gen2 CUID tags have `SAK: 81` and `ATQA: 0004`. Those will be recognized as "**Not a MIFARE Classic tag!**" after the first write and most software won't be able to re-write them.
